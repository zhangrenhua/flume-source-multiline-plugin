 ...
 * agent.sources.r1.interceptors.i1.serializers.s2.type = com.urey.flume.RegexExtractorEnhanceInterceptorTimestampSerializer
 ...
 * </pre>
 */
public class RegexExtractorEnhanceInterceptor implements Interceptor {
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(RegexExtractorEnhanceInterceptor.class);

    private final Pattern regex;
    private final List<RegexExtractorEnhanceInterceptor.NameAndSerializer> serializers;

    private RegexExtractorEnhanceInterceptor(Pattern regex,
                                      List<RegexExtractorEnhanceInterceptor.NameAndSerializer> serializers) {
        this.regex = regex;
    ...
    public Event intercept(Event event) {
        ...
        if (matcher.find()) {
            for (int group = 0, count = matcher.groupCount(); group < count; group++) {
                ...
                RegexExtractorEnhanceInterceptor.NameAndSerializer serializer = serializers.get(group);
                if (logger.isDebugEnabled()) {
    ...
    public static class Builder implements Interceptor.Builder {
        ...
        private List<RegexExtractorEnhanceInterceptor.NameAndSerializer> serializerList;
        ...
        public void configure(Context context) {
            ...
            // 增加flags参数
            int flags = context.getInteger(FLAGS, 0);
            regex = Pattern.compile(regexString,flags);
        ...
        private void configureSerializers(Context context) {
            ...
            for (String serializerName : serializerNames) {
                ...
                if ("DEFAULT".equals(type)) {
                    serializerList.add(new RegexExtractorEnhanceInterceptor.NameAndSerializer(name, defaultSerializer));
                } else {
                    serializerList.add(new RegexExtractorEnhanceInterceptor.NameAndSerializer(name, getCustomSerializer(
        ...
        public Interceptor build() {
            ...
            return new RegexExtractorEnhanceInterceptor(regex, serializerList);
        }
    }
