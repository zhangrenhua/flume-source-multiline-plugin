...
public class MultiLineExecSource extends AbstractSource implements EventDrivenSource, Configurable {
    ...
    public void start() {
        ...
        runner = new ExecRunnable(shell, command, getChannelProcessor(), sourceCounter,
                restart, restartThrottle, logStderr, bufferCount, batchTimeout, charset, regex, lineMaxSize);
    ...
    public void configure(Context context) {
        ...
        lineMaxSize = context.getInteger(MultiLineExecSourceConfigurationConstants.LINE_MAX_SIZE, 1024 * 1024 * 2);
    ...
    private static class ExecRunnable implements Runnable {
        ...
        ExecRunnable(String shell, String command, ChannelProcessor channelProcessor,
                     SourceCounter sourceCounter, boolean restart, long restartThrottle,
                     boolean logStderr, int bufferCount, long batchTimeout, Charset charset, String regex, int lineMaxSize) {
        ...
        public void run() {
            do {
                ...
                final List<Event> eventList = new ArrayList<Event>(bufferCount);
                ...
                try {
                    ...
                    reader = new BufferedInputStream(process.getInputStream(), 1024);
                    ...
                    int len;
                    StringBuilder recordBuffer = new StringBuilder(1024);

                    while ((len = reader.read(buffer)) != -1) {
                        synchronized (eventList) {
                            String tmp = new String(buffer, 0, len);
                            recordBuffer.append(tmp);
                            ...
                            // 长度控制
                            if (recordBuffer.length() >= lineMaxSize) {
                                logger.warn("recordBuffer.length() >= lineMaxSize ,recordBuffer: {}", recordBuffer);
                                recordBuffer = new StringBuilder(1024);
                                continue;
                            }

                            //multiline setting start
                            String records = recordBuffer.toString();
                            Matcher m = pattern.matcher(records);
                            int lastStart = -1;

                            while (m.find()) {
                                if (lastStart != -1) {
                                    eventList.add(EventBuilder.withBody(records.substring(lastStart, m.start()).getBytes(charset)));
                                }
                            ...
                            recordBuffer = new StringBuilder()
                                    .append(recordBuffer.substring(lastStart));
                    ...
                    synchronized (eventList) {
                        if (recordBuffer.length() > 0) {
                            //multiline setting start
                            eventList.add(EventBuilder.withBody(recordBuffer.toString().getBytes(charset)));
                        }
    ...
    private static class StderrReader extends Thread {
        ...
        StderrReader(BufferedReader input, boolean logStderr) {
            this.input = input;
            this.logStderr = logStderr;
        }
